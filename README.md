...
See the `Tasks and parameter` section for details on all tasks and psettings available in `Sketchy`.
...
Template sketches can be called for:

* *Staphylococcus aureus*: `saureus`
* *Klebsiella pneumoniae*: `kpneumoniae`
* *Mycobacterium tuberculosis*: `mtuberculosis`

When using a template for the included pathogen databases with `--sketch`, execution looks like this:
...
Non-template reference sketch collections generated by the `sketchy feature prepare`, and must include a reference sketch, genotype index and key file with the same file name and the following extensions, such that:
...
Non-template collections can be called with reference in the `--sketch` parameter:
