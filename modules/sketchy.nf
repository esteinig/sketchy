...
process SketchyScreenWinner {
    ...
    publishDir "${params.outdir}/screen_winner/${db.baseName}/${read_limit}", mode: "copy", pattern: "${id}.tsv"
    ...
    """
    SKETCHY_PATH=\$PWD
    head -$_read_limit $fx | sketchy screen --fastx - --db $db --limit $params.limit --threads $task.cpus --winner > ${id}.tsv
    sketchy head --db $db > header.txt
    """
